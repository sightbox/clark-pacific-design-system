...
const texts=page.findAllWithCriteria({types:['TEXT']});const own=texts.filter(n=>!inInstance(n));
const agg={};let maxd=[];
for(const n of own){const {m}=mainSeg(n);const bp=bpOf(n);const {s,d}=pick(m.fontName,m.fontSize,m.lineHeight,m.letterSpacing,m.textCase,bp);
  const k=`${m.fontName.style} ${m.fontSize}/${Math.round(lhpx(m.lineHeight,m.fontSize,m.fontName.family)*10)/10} ls${Math.round(m.letterSpacing.value*10)/10} ${m.textCase[0]} ${bp[0]} -> ${s?s.name:'NONE'} (${s?`${s.fontName.style} ${s.fontSize}/${Math.round(lhpx(s.lineHeight,s.fontSize,s.fontName.family)*10)/10} ls${s.letterSpacing.value} ${s.textCase[0]}`:''}) d=${d.toFixed(1)}`;agg[k]=(agg[k]||0)+1;}
return {own:own.length,inInst:texts.length-own.length,map:Object.entries(agg).sort((a,b)=>parseFloat(b[0].split('d=')[1])-parseFloat(a[0].split('d=')[1])).map(([k,v])=>v+'× '+k).join('\n')};
