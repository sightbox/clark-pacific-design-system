...
async function build(n,parent){
  ...
  if(n.t==='T'){
    ...
    node.characters=n.c;
    const st=TS[n.s];if(st)await node.setTextStyleIdAsync(st.id);
    node.fontName={family:n.ff||'Poppins',style:fstyle(n.ff,n.fw,n.it)};
    node.fontSize=n.fs;
  ...
  if(n.t==='C'){
    ...
    if(n.icon){const s=n.w/24;if(Math.abs(s-1)>0.001)node.rescale(s);
      ...
      for(const v of node.findAll(x=>x.type==='VECTOR'||x.type==='ELLIPSE'||x.type==='RECTANGLE'||x.type==='POLYGON'||x.type==='LINE')){
        ...
        if(n.sw&&v.strokes&&v.strokes.length)v.strokeWeight=n.sw*s;
      }
  ...
  node.resize(Math.max(0.01,n.w),Math.max(0.01,n.h));
  if(n.abs){node.layoutPositioning='ABSOLUTE';node.x=n.abs[0];node.y=n.abs[1];}
  if(n.tf){const m=n.tf.match(/rotate\(([-\d.]+)deg\)/)||n.tf.match(/matrix\(([-\d.e]+), ([-\d.e]+)/);if(m){const deg=m[2]!==undefined?Math.atan2(+m[2],+m[1])*180/Math.PI:+m[1];if(Math.abs(deg)>0.1){const cx=node.x+node.width/2,cy=node.y+node.height/2;node.rotation=-deg;node.x=cx-(node.width*Math.cos(-deg*Math.PI/180)-node.height*Math.sin(-deg*Math.PI/180))/2;}}}
...
async function prep(roots,pageName){
  fonts.clear();TEXTPROPS.length=0;SHOWPROPS.length=0;unbound.length=0;
  for(const r of roots)scan(r);
  const pg=figma.root.children.find(p=>p.name===pageName);await figma.setCurrentPageAsync(pg);
...
function stashGet(key){let s='',i=0,part;while((part=figma.root.getSharedPluginData('cpbuild','chunk:'+key+':'+i))){s+=part;i++;}return s;}
function stashClear(key){let i=0;while(figma.root.getSharedPluginData('cpbuild','chunk:'+key+':'+i)){figma.root.setSharedPluginData('cpbuild','chunk:'+key+':'+i,'');i++;}}
async function buildSet({ROOTS,GROUP,SETNAME,DESC,STASH,IMGS}){
  if(STASH){ROOTS=JSON.parse(stashGet(STASH));}
  IMG=IMGS||{};
  const pg=await prep(ROOTS,"02 · Components");
  let sec=pg.children.find(n=>n.type==='SECTION'&&n.name===GROUP);
  if(!sec){sec=figma.createSection();sec.name=GROUP;pg.appendChild(sec);const right=Math.max(0,...pg.children.filter(n=>n!==sec).map(n=>n.x+n.width));sec.x=right+200;sec.y=0;sec.resizeWithoutConstraints(1800,400);}
  const comps=[];
  for(const r of ROOTS){const c=await build(r,sec);comps.push(c);}
  const set=figma.combineAsVariants(comps,sec);set.name=SETNAME;set.description=DESC||'';
  const keyOf={};
  for(const [node,pn] of TEXTPROPS){if(!keyOf[pn])keyOf[pn]=set.addComponentProperty(pn,'TEXT',node.characters);node.componentPropertyReferences={characters:keyOf[pn]};}
  for(const [node,sp] of SHOWPROPS){if(!keyOf[sp])keyOf[sp]=set.addComponentProperty(sp,'BOOLEAN',true);node.componentPropertyReferences={...node.componentPropertyReferences,visible:keyOf[sp]};}
  for(const [node] of SHOWPROPS)node.isExposedInstance=true;
  set.layoutMode='HORIZONTAL';set.layoutWrap='WRAP';set.itemSpacing=80;set.counterAxisSpacing=80;set.counterAxisAlignItems='MIN';set.paddingTop=set.paddingBottom=set.paddingLeft=set.paddingRight=40;
  const totalW=comps.reduce((a,c)=>a+c.width+80,40);set.resize(Math.min(Math.max(totalW,400),3400),100);set.primaryAxisSizingMode='FIXED';set.counterAxisSizingMode='AUTO';
  await set.setFillStyleIdAsync(PS['Neutral/BG Alt'].id);
  const others=sec.children.filter(n=>n!==set);const bottom=others.length?Math.max(...others.map(n=>n.y+n.height)):0;
  set.x=80;set.y=bottom+120;
  sec.resizeWithoutConstraints(Math.max(sec.width,set.x+set.width+80),set.y+set.height+80);
  if(STASH)stashClear(STASH);
  const out={set:set.id,variants:Object.fromEntries(set.children.map(c=>[c.name,c.id])),props:Object.keys(set.componentPropertyDefinitions),unbound:[...new Set(unbound)],sizes:set.children.map(c=>[c.name,Math.round(c.width),Math.round(c.height)])};
  return {out,set};
}
function stash(key,i,part){figma.root.setSharedPluginData('cpbuild','chunk:'+key+':'+i,part);return part.length;}
return {build,prep,buildSet,stash,stashGet,stashClear,PS,TS,unbound,TEXTPROPS,SHOWPROPS,setIMG:(x)=>{IMG=x;}};

}
figma.root.setSharedPluginData("cpbuild","lib",CPLIB.toString());
const lib=await eval("("+figma.root.getSharedPluginData("cpbuild","lib")+")")(figma);
return {stored:CPLIB.toString().length,ok:typeof lib.buildSet};